...
const exit_button_artikli = window.document.querySelector(".exit-modal-btn-3");
const modal = window.document.querySelector(".modal");
const overlay = window.document.querySelector(".overlay");
const modal_artikli = window.document.querySelector(".modal_artikli");
const overlay_artikli = window.document.querySelector(".overlay_artikli");
...
//exit funkcija kod ispisa artikala
exit_button_artikli.addEventListener("click", function () {
  modal_artikli.classList.add("hidden");
  overlay_artikli.classList.add("hidden");
...
button_add.addEventListener("click", function () {
  if (button_add_position === 0) {
    try {
      ...
      novi.artikli = add_artikli(artikli_popis);
...
window.document
  ...
  .addEventListener("click", function (e) {
    ...
    if (e.target.classList.value === "artikli") {
      const parent = e.target.closest(".movements__row");
      modal_artikli.classList.remove("hidden");
      overlay_artikli.classList.remove("hidden");
      const data = customHttp(
        `http://localhost:3000/items/${parent.id}`,
        "GET"
      );
